...
const getEmotionTypeByResults = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const traumeticExpModel = (0, tests_data_schema_1.getUniversalModel)('traumatic-experience-results');
        let data = yield traumeticExpModel.find();
        if (!data || data.length === 0) {
            const googlefileData = yield google_sheets_1.default.getDataGoogle(fileId);
            yield traumeticExpModel.create(googlefileData);
            data = yield traumeticExpModel.find();
        }
        const results = data[0][emotionType];
        res.status(200).send({
