...
const getQuestions = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const fileId = google_file_ids_env_1.TOXICAL_RELATIONSHIPS.QUESTIONS;
        const toxicalRelationshipsModel = (0, tests_data_schema_1.getUniversalModel)('toxical-relationships-questions');
        let data = yield toxicalRelationshipsModel.find();
        if (!data || data.length === 0) {
            const questions = (yield google_sheets_1.default.getDataGoogle(fileId));
            yield toxicalRelationshipsModel.create(questions);
            data = yield toxicalRelationshipsModel.find();
        }
        //
        const normalizeId = data.map((i, index) => (Object.assign({ id: index + 1 }, i._doc)));
        res.status(200).send({
            message: 'Success get toxical-relationships questions!',
